...
{
    OPEN_LOOP           = 0,                // 0b0000
    CURRENT_LOOP        = (1U << 0),        // 0b0001
    SPEED_LOOP          = (1U << 1),        // 0b0010
    ANGLE_LOOP          = (1U << 2),        // 0b0100

    // only for checking (这些是组合值，非独立位，保留为常量即可)
    SPEED_AND_CURRENT_LOOP = CURRENT_LOOP | SPEED_LOOP,   // 0b0011
    ANGLE_AND_SPEED_LOOP   = ANGLE_LOOP   | SPEED_LOOP,   // 0b0110
    ALL_THREE_LOOP         = CURRENT_LOOP | SPEED_LOOP | ANGLE_LOOP, // 0b0111
} Closeloop_Type_e;


typedef enum
{
    FEEDFORWARD_NONE                     = 0,                         // 0b00
    CURRENT_FEEDFORWARD                  = (1U << 0),                // 0b01
    SPEED_FEEDFORWARD                    = (1U << 1),                // 0b10
    CURRENT_AND_SPEED_FEEDFORWARD        = CURRENT_FEEDFORWARD | SPEED_FEEDFORWARD, // 0b11
} Feedforward_Type_e;
