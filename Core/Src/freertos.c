...
void MX_FREERTOS_Init(void) {
  ...
  osThreadDef(MotorControl, MotorControl_task, osPriorityRealtime, 0, 256);
  MotorControlHandle = osThreadCreate(osThread(MotorControl), NULL);

  /* definition and creation of PostureControl */
  osThreadDef(PostureControl, PostureControl_task, osPriorityRealtime, 0, 512);
...
/* USER CODE BEGIN Header_PostureControlTask */
/**
* @brief Function implementing the PostureControl thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_PostureControlTask */
__weak void PostureControl_task(void const * argument)
{
  /* USER CODE BEGIN PostureControlTask */
  /* Infinite loop */
  for(;;)
  {
    osDelay(1);
  }
  /* USER CODE END PostureControlTask */
}
