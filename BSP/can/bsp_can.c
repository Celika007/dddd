...
/* 包含 CAN BSP 头文件，提供实例结构与宏类型选择 */
#include "bsp_can.h"

/* 包含主工程头文件，提供硬件句柄等全局符号 */
#include "main.h"

/* 包含内存操作相关定义（malloc/free 等） */
#include "memory.h"

/* 标准库，提供动态内存分配与基础类型支持 */
#include "stdlib.h"

/* DWT 计时器封装，提供毫秒时间戳与周期测量 */
#include "bsp_dwt.h"
...
**********************************************************************
  * @ 函数名  ： CANAddFilter
  * @ 功能说明： 为指定 CAN 实例添加接收过滤器，按 rx_id 分配到 FIFO
  * @ 参数    ： _instance  指向 CANInstance 的指针
  * @ 返回值  ： 无
  * @ 涉及变量： 过滤器索引（不同总线独立维护）
********************************************************************/
/*
...
**********************************************************************
  * @ 函数名  ： CANServiceInit
  * @ 功能说明： 第一次注册实例时初始化 CAN 外设并开启接收中断
  * @ 参数    ： 无
  * @ 返回值  ： 无
  * @ 涉及变量： FDCAN/BxCAN 不同通知与模式配置
********************************************************************/
/*
***************************函数专属变量******************************
*/
...
**********************************************************************
  * @ 函数名  ： CANRegister
  * @ 功能说明： 注册并初始化一个 CAN 实例，配置发送头与接收过滤器
  * @ 参数    ： config  指向初始化配置结构体的指针
  * @ 返回值  ： CANInstance*  返回实例指针供模块保存
  * @ 涉及变量： can_instance[] 实例表、idx 实例索引
********************************************************************/
/*
***************************函数专属变量******************************
*/
...
**********************************************************************
  * @ 函数名  ： CANTransmit
  * @ 功能说明： 阻塞式发送，等待 Tx FIFO/邮箱空闲直至超时或成功
  * @ 参数    ： _instance  CAN 实例指针；timeout 超时（ms）
  * @ 返回值  ： 1 成功；0 超时或 HAL 失败
  * @ 涉及变量： busy_count（失败累计），wait_time（等待耗时）
********************************************************************/
/*
***************************函数专属变量******************************
*/
...
**********************************************************************
  * @ 函数名  ： CANSetDLC
  * @ 功能说明： 设置发送数据帧长度（1~8 字节），FDCAN 进行枚举映射
  * @ 参数    ： _instance  CAN 实例指针；length 帧长度
  * @ 返回值  ： 无（非法长度进入错误处理）
  * @ 涉及变量： txconf.DataLength 或 txconf.DLC
********************************************************************/
/*
***************************函数专属变量******************************
*/
void CANSetDLC(CANInstance *_instance, uint8_t length)
{
    ...
    _instance->txconf.DataLength = fdcan_dlc_map[length];
#else
...
/*
**********************************************************************
  * @ 函数名  ： FDCANFIFOxCallback
  * @ 功能说明： FDCAN 接收回调处理，遍历匹配实例并复制数据到缓冲
  * @ 参数    ： _hfdcan FDCAN 句柄；fifox 指示 FIFO0/1
  * @ 返回值  ： 无
  * @ 涉及变量： rxconf（接收头）、fdcan_rx_buff（临时缓冲）、idx（实例数）
********************************************************************/
/*
***************************函数专属变量******************************
*/
...
**********************************************************************
  ...
  * @ 功能说明： FDCAN FIFO0 接收中断回调，检查标志并转发到处理函数
  * @ 参数    ： hfdcan FDCAN 句柄；RxFifo0ITs 中断标志集合
  * @ 返回值  ： 无
  * @ 涉及变量： 无
********************************************************************/
/*
...
/*
**********************************************************************
  * @ 函数名  ： HAL_FDCAN_RxFifo1Callback
  * @ 功能说明： FDCAN FIFO1 接收中断回调，检查标志并转发到处理函数
  * @ 参数    ： hfdcan FDCAN 句柄；RxFifo1ITs 中断标志集合
  * @ 返回值  ： 无
  * @ 涉及变量： 无
********************************************************************/
/*
...
/*
**********************************************************************
  * @ 函数名  ： CANFIFOxCallback
  * @ 功能说明： BxCAN 接收回调处理，遍历匹配实例并复制数据到缓冲
  * @ 参数    ： _hcan CAN 句柄；fifox 指示 FIFO0/1
  * @ 返回值  ： 无
  * @ 涉及变量： rxconf（接收头）、can_rx_buff（临时缓冲）、idx（实例数）
********************************************************************/
/*
***************************函数专属变量******************************
*/
...
**********************************************************************
  ...
  * @ 功能说明： BxCAN FIFO0 接收中断回调，转发到自定义处理函数
  * @ 参数    ： hcan CAN 句柄
  * @ 返回值  ： 无
  * @ 涉及变量： 无
********************************************************************/
...
**********************************************************************
  ...
  * @ 功能说明： BxCAN FIFO1 接收中断回调，转发到自定义处理函数
  * @ 参数    ： hcan CAN 句柄
  * @ 返回值  ： 无
  * @ 涉及变量： 无
********************************************************************/
...
**********************************************************************
  * @ 函数名  ： CANTransmitTry
  * @ 功能说明： FDCAN 下非阻塞发送，若队列空闲则立即发送
  * @ 参数    ： _instance  CAN 实例指针
  * @ 返回值  ： 1 成功；0 队列占满或 HAL 失败
  * @ 涉及变量： 无
********************************************************************/
/*
***************************函数专属变量******************************
...
**********************************************************************
  * @ 函数名  ： CANTransmitEx
  * @ 功能说明： FDCAN 下零拷贝发送，按长度映射 DLC 并发送外部缓冲
  * @ 参数    ： _instance  实例；data 数据指针；length 字节长度
  * @ 返回值  ： 1 成功；0 参数非法/队列占满/HAL 失败
  * @ 涉及变量： 无
********************************************************************/
/*
***************************函数专属变量******************************
...
uint8_t CANTransmitEx(CANInstance *_instance,const uint8_t *data,uint8_t length)
{
    ...
    if (HAL_FDCAN_GetTxFifoFreeLevel(_instance->can_handle) == 0)
        return 0;
    if (HAL_FDCAN_AddMessageToTxFifoQ(_instance->can_handle, &_instance->txconf, (uint8_t*)data))
        return 0;
    return 1;
}
#else
...
**********************************************************************
  * @ 函数名  ： CANTransmitTry
  * @ 功能说明： BxCAN 下非阻塞发送，若邮箱空闲则立即发送
  * @ 参数    ： _instance  CAN 实例指针
  * @ 返回值  ： 1 成功；0 邮箱占满或 HAL 失败
  * @ 涉及变量： 无
********************************************************************/
/*
***************************函数专属变量******************************
...
**********************************************************************
  * @ 函数名  ： CANTransmitEx
  * @ 功能说明： BxCAN 下零拷贝发送，设置 DLC 并发送外部缓冲
  * @ 参数    ： _instance  实例；data 数据指针；length 字节长度
  * @ 返回值  ： 1 成功；0 参数非法/邮箱占满/HAL 失败
  * @ 涉及变量： 无
********************************************************************/
/*
***************************函数专属变量******************************
...
uint8_t CANTransmitEx(CANInstance *_instance,const uint8_t *data,uint8_t length)
{
    ...
    if (HAL_CAN_GetTxMailboxesFreeLevel(_instance->can_handle) == 0)
        return 0;
    if (HAL_CAN_AddTxMessage(_instance->can_handle, &_instance->txconf, (uint8_t*)data, &_instance->tx_mailbox))
        return 0;
    return 1;
}
#endif
